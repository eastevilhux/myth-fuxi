...
class BaseResponseBodyConverter<T> internal constructor(
    ...
    override fun convert(value: ResponseBody): T {
        ...
        SLog.longD(TAG,encryption.toString());
        var result = Result<T>();
        result.code = code;
        ...
        SLog.longD(TAG,"RESULT_DATA=>${dataStr}")

        if(encryption){
            //需要数据解密
            if(dataStr != null && dataStr.isNotEmpty()){
                SLog.longD(TAG,"NEED_ENCRYPT,DATA IS ==>${dataStr}");
                //判断是否需要进行URLDecode
                if(result.urlEncoder){
                    dataStr = URLDecoder.decode(dataStr,NetworkHelper.instance().httpConfig().charset());
                    SLog.longD(TAG,"URLECODE_RESULT==>${dataStr}");
                }else{
                ...
                //进行数据解密处理
                if(dataStr != null){
                    dataStr = DataHelper.instance.dataSecurity()?.aesDecrypt(dataStr!!);
                    SLog.longD(TAG,"RESULT_DECRPYT=>${dataStr}")
            ...
            SLog.longD(TAG,"BEFOR_TOJSON_DATA==>${dataStr}");
            val type: Type = object : TypeToken<T>() {}.type
            var t = gson.fromJson<T>(dataStr,type);
            result.data = t;
            var gsonData = gson.toJson(result);
            SLog.longD(TAG,"HTTP_BASE_RESULT=>${gsonData}");
            val reader = StringReader(gsonData);
            return adapter.fromJson(reader)
        }
    }
